...
def update_cache(breed_ids, cache_file):
    ...
    for breed_id in breed_ids:
        ...
        if result is not None:
            ...
            response_url = result[1]
            
            cache[response_url] = parsed_json
            
            successful_new_adds = successful_new_adds + 1
    
    create_cache(cache, cache_file)
    

    total_breeds = len(breed_ids)
    percentage = (successful_new_adds / total_breeds) * 100
    
    result_string = "Cached data for " + str(percentage) + "% of breeds"
...
def get_longest_lifespan_breed(cache_file):
    ...
    for url, entry in cache.items():
        ...
        max_lifespan = life["max"]
        
        if type(max_lifespan) != int:
            continue  
        
        valid_breeds.append((name, max_lifespan))
    
    if len(valid_breeds) == 0:
        return "No breeds found"
    

    best_name = valid_breeds[0][0]
    best_lifespan = valid_breeds[0][1]
    ...
    for breed in valid_breeds:
        ...
        if lifespan > best_lifespan:
            best_name = name
            best_lifespan = lifespan
        elif lifespan == best_lifespan:
            if name < best_name:
                best_name = name
                best_lifespan = lifespan
